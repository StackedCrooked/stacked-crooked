...
{
public:
    Client(io_service & io_service, tcp::resolver::iterator endpoint_iterator)
    ...
    {
        io_service_.post(bind(&Client::do_write, this, msg));
    }

    void close()
    {
        io_service_.post(bind(&Client::do_close, this));
    }
    ...
    void handle_connect(const system::error_code & error)
    {
        if (!error)
        {
            async_read(socket_,
                       ...
                       bind(&Client::handle_read_header, this,
                            placeholders::error));
        }
    }
    ...
    void handle_read_header(const system::error_code & error)
    {
        if (!error && read_msg_.decode_header())
        {
            async_read(socket_,
                       ...
                       bind(&Client::handle_read_body, this,
                            placeholders::error));
        }
        else
        {
