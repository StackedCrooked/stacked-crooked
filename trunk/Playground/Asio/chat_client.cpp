...
{
public:
    chat_client(io_service & io_service,
                tcp::resolver::iterator endpoint_iterator)
        : io_service_(io_service),
          socket_(io_service)
    {
        async_connect(socket_, endpoint_iterator,
                      bind(&chat_client::handle_connect, this,
                           placeholders::error));
    }

    void write(const chat_message & msg)
    {
        io_service_.post(bind(&chat_client::do_write, this, msg));
    }

    void close()
    {
        io_service_.post(bind(&chat_client::do_close, this));
    }
    ...
    void handle_connect(const system::error_code & error)
    {
        if (!error)
        {
            async_read(socket_,
                       ...
                       bind(&chat_client::handle_read_header, this,
                            placeholders::error));
        }
    }
    ...
    void handle_read_header(const system::error_code & error)
    {
        if (!error && read_msg_.decode_header())
        {
            async_read(socket_,
                       ...
                       bind(&chat_client::handle_read_body, this,
                            placeholders::error));
        }
        else
        {
    ...
    void do_write(chat_message msg)
    {
        ...
        if (!write_in_progress)
        {
            async_write(socket_,
                        ...
                        bind(&chat_client::handle_write, this,
                             placeholders::error));
        }
    }
        ...
        if (!error)
        {
            ...
            if (!write_msgs_.empty())
            {
                async_write(socket_,
                            ...
                            bind(&chat_client::handle_write, this, placeholders::error));
            }
        }
        else
        {
...
int main(int argc, char * argv[])
{
    try
    {
        ...
        thread t(bind(&io_service::run, &io_service));
